...
//#include "hiduser.h"				 //kk

extern 	void USB_DataOutStage_EP1(void);		//kk
...
void USB_Configure_Event (void) {
  ...
  if (USB_Configuration) {                   /* Check if USB is configured */
    ...
    GetInReport();	//kk
//    USB_WriteEP(HID_EP_IN, &InReport, sizeof(InReport));//kk
...
void USB_EndPoint1 (U32 event) {
  ...
  switch (event) {
    case USB_EVT_IN:	 //#define USB_EVT_IN 	3   /* IN Packet */
      GetInReport();
	  if (InReport==0){break;}		//kangkai
//      USB_WriteEP(HID_EP_IN, &InReport, sizeof(InReport));
      break;
	//---------kangkai---------// 
	case USB_EVT_OUT:
		USB_DataOutStage_EP1();
//		printf("EP1 Count=%x \n",EP1Data.Count);
		break;
	//---------kangkai ---------//		
	default: 
