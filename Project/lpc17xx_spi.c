...
void SPI_Init (void) 
{
   ...
   PCONP |= (1 << 21);
    ...
    PINSEL1 &= ~(3 << 0);          /* Configure P0.16(SSEL) as GPIO */
    FIO0DIR |= (1 << 16);            /* set P0.16 as output */
    ...
    PCLKSEL1 &= ~(3 << 10);
   PCLKSEL1 |=  (1 << 10);  /* SSP0_PCLK=CCLK */
    
    /* 8bit, SPI frame format, CPOL=0, CPHA=0, SCR=0 */  
    SSP0CR0 = (0x07 << 0) |     /* data width: 8bit*/
