...
extern void WrCmdDat (U32 cmd, U32 val);				 //kk
...
U8  EP1Buf[USB_MAX_PACKET1];				   //kk

USB_EP_DATA EP0Data;
USB_EP_DATA EP1Data;						   //kk
...
//------------kangkai-----------
void USB_DataOutStage_EP1 (void) {
  U32 cnt;

  	cnt = USB_ReadEP(0x01, EP1Buf);		   
  	if ((cnt > 1)&&(cnt <= 64))
  	{
      OutReport = EP1Buf[0];
      SetOutReport();
          ...
          switch (pD->bDescriptorType) {
            ...
            case USB_ENDPOINT_DESCRIPTOR_TYPE:
              if (alt == 0) {
                ...
                USB_EnableEP(n);
				WrCmdDat(CMD_SET_EP_STAT(2), DAT_WR_BYTE(0));		//kk
                USB_ResetEP(n);
				WrCmdDat(CMD_SET_EP_STAT(2), DAT_WR_BYTE(0));		//kk
              }
              break;
