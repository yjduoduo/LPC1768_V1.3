...
#line 86 "..\\src\\Hardware\\Timer\\CTimeDef.h"
...
#line 1 "..\\src\\Share\\CSetCurrent.h"
...
void CirAddrCounter(void)
{
	...
	if(syn_flag)
	{
		...
		{			
			...
			Check_Addr(GetCirCounter());	
			if(GetCirCounter() == 219 ) 
			{
				 syn_flag = 0;
			}		
		}
		else if((GetTimer0Counter() > 100)&&(GetTimer0Counter() < 135))
		{
			AddCirCounter(); 
			ClrTimer0Counter();	
 			Sel_Addr_fire(GetCirCounter());
		...
		else if((GetTimer0Counter() > 140)&&(GetTimer0Counter() < 170)) 
		{
			AddCirCounter(); 
			ClrTimer0Counter();	
			if(GetCirCounter() == 219 ) 
		...
		else if((GetTimer0Counter() > 180)&&(GetTimer0Counter() < 210)) 
		{
			AddCirCounter(); 
			ClrTimer0Counter();	
			if(GetCirCounter() == 219) 
...
void Reset_Alarm(void)
{
	...
	if(Compare_Address(GetCirCounter()))
	{
		...
		if(Reset_Flag1[GetCirCounter()-GetSendToF(0)])
		{
			...
			{
				if((*(volatile unsigned long *)(0x2009C000 + 0x14))&(1 << 23))
...
void Check_Addr(uint8 Current_Addr)
{
	...
	{
			if(GetCirAlarmData(Current_Addr-GetSendToF(0)+1)==0x31)
			{
				State_0mA();
			}
			else if(GetCirAlarmData(Current_Addr-GetSendToF(0)+1)==0x32)
			{
				State_0mA();
			}
			else if(GetCirAlarmData(Current_Addr-GetSendToF(0)+1)==0x33)
...
void Sel_Addr_fire(uint8 Current_Addr)
{
	...
	if(Compare_Address(Current_Addr))
	{
		if(GetCirAlarmData(Current_Addr-GetSendToF(0)+1)==0x33)
...
void CirSendFireInfo(void)
{
	if(fireflag[1])
	{
		if(GetTimer0Counter() == 17) 
		{
			State_0mA();
		}
		
		if(GetTimer0Counter() == 23) 
		{
			State_20mA();
		}
		if(GetTimer0Counter() == 35)  
		{
			State_6mA();
		}
		if(GetTimer0Counter() == 41) 
		{
			State_20mA();
		}
		if(GetTimer0Counter() == 47)
		{
			State_6mA();
		}
		if(GetTimer0Counter() == 53)
		{
			State_20mA();
		}
