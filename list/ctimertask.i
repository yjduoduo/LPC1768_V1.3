...
#line 17 "..\\src\\Share\\CTimerTask.c"
#line 18 "..\\src\\Share\\CTimerTask.c"
#line 19 "..\\src\\Share\\CTimerTask.c"
...
void TickTaskWork (void)   
{
uint8 j;
uint32 ii;
if(GetTimeTickCounter()==10)
{
	...
	if(d100msTimer >= 100)
	{
		...
		for(j=20; j<40; j++)
		{
			...
			if(dN00msTimer >= stTaskTimer[j].OrgMSec)
				...
				ii = (0x10000 - 1) - stTaskTimer[j].OrgMSec + 1 + dN00msTimer;
			if(ii >= stTaskTimer[j].SetMSec)
			{
	...
	if(d1sTimer >= 1000)
	{
		...
		for(j=40; j<50; j++)
		{
			...
			if(dNsTimer >= stTaskTimer[j].OrgMSec)
				...
				ii = (0x10000 - 1) - stTaskTimer[j].OrgMSec + 1 + dNsTimer;
			if(ii >= stTaskTimer[j].SetMSec)
			{
